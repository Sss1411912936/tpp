...
class ViewerOrdersAPIView(ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # 将要买的座位先去redis中查询座位，如果座位又占用，直接返回，如果没有占用，我们要存数据占用座位，
        # 之后进行购买。要有过期时间；若取消订单，则释放座位

        v_seats_list = v_seats.split('#')
        for v_seat in v_seats_list:
            result = cache.get(v_paidang_id + ' lock ' + v_seat)
            if result:
                raise APIException(detail='锁单失败')
        # 所有预定位置可用，存入redis
        for v_seat in v_seats_list:
